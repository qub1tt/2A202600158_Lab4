...
def _format_price(amount: int) -> str:
    """Format số tiền theo định dạng Việt Nam: 1.450.000đ"""
...
def search_flights(origin: str, destination: str) -> str:
    ...
    # Thử tra ngược chiều nếu không có
    if not flights:
        flights = FLIGHTS_DB.get((destination, origin))
        if flights:
            # Đây là chiều ngược — thông báo rõ
            return (
                f"Lưu ý: Không có dữ liệu tuyến {origin} → {destination}. "
                f"Hiển thị tuyến ngược {destination} → {origin}:\n"
                + _format_flights(flights)
            )

    if not flights:
        return f"Không tìm thấy chuyến bay từ {origin} đến {destination}. Hiện tại hệ thống hỗ trợ các tuyến: Hà Nội ↔ Đà Nẵng, Hà Nội ↔ Phú Quốc, Hà Nội ↔ Hồ Chí Minh, Hồ Chí Minh ↔ Đà Nẵng, Hồ Chí Minh ↔ Phú Quốc."

    return f"Các chuyến bay từ {origin} đến {destination}:\n" + _format_flights(flights)
...
def search_hotels(city: str, max_price_per_night: int = 99_999_999) -> str:
    ...
    if not hotels:
        return f"Không tìm thấy dữ liệu khách sạn tại {city}. Hệ thống hỗ trợ: Đà Nẵng, Phú Quốc, Hồ Chí Minh."
    ...
    for i, h in enumerate(filtered, 1):
        stars_str = "★" * h["stars"]
        lines.append(
            f"  {i}. {h['name']} {stars_str} | {_format_price(h['price_per_night'])}/đêm | "
...
def calculate_budget(total_budget: int, expenses: str) -> str:
        ...
        if expenses.strip():
            for item in expenses.split(","):
                ...
                if not item:
                    continue
                if ":" not in item:
                    return f"Lỗi định dạng: '{item}' không đúng cú pháp 'tên:số_tiền'. VD: 've_may_bay:890000,khach_san:650000'"
                name, amount_str = item.rsplit(":", 1)
                name = name.strip().replace("_", " ")
                amount_str = amount_str.strip().replace(".", "").replace(",", "")
                expense_dict[name] = int(amount_str)
    except ValueError as e:
        return f"Lỗi khi đọc số tiền: {e}. Hãy đảm bảo số tiền là số nguyên (VD: 890000)."
    ...
    lines.append("  ---")
    lines.append(f"  Tổng chi: {_format_price(total_expense)}")
    lines.append(f"  Ngân sách: {_format_price(total_budget)}")

    if remaining >= 0:
        lines.append(f"  Còn lại: {_format_price(remaining)}")
    else:
        lines.append(f"  VƯỢT NGÂN SÁCH: {_format_price(abs(remaining))}! Cần điều chỉnh kế hoạch.")
